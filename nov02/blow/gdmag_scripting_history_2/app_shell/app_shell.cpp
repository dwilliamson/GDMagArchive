...
unsigned char *do_dumb_alpha_thing(unsigned char *src, int width, int height) {
    ...
    for (i = 0; i < npixels; i++) {
        ...
        int sum = r + g + b;
        float opacity = (float)(sum / 90.0f);
        if (opacity < 0.33f) opacity = 0;
        if (opacity > 1) opacity = 1;
        int a = (int)(opacity * 255.0f + 0.5f);
...
void App_Shell::sleep(double seconds) {
...
// strdup is defined here because the stdlib version crashes on Win32 when you 
// try to free, in the case of a library conflict (linking against debug + 
// release libraries) but I want to live with library conflicts so I can ship 
// this code with just one version of jpg.lib and zlib.lib and people can just
// link and be happy.
char *App_Shell::strdup(char *string) {
