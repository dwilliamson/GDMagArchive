...
void combine_tree(World_Block *node) {
    World_Block **n = node->children;
...
Mesh_Seam *xref_seam(Mesh_Seam *source, World_Block *block, int mesh_index) {
...
inline World_Block *get_child_block(World_Block *parent,
                                             int index) {
    if (parent->children[index]) return parent->children[index];
    return NULL;
}


void add_child(World_Block *parent, World_Block *child) {
    if (!child) return;
...
void do_rewrite_rule(World_Block *source_block,
    ...
    seam_database->perform_rewrite_rule(&rule, 1);
}

World_Block *build_tree_upward(World_Block *nodes,
    ...
    World_Block *parents = new World_Block[num_parents];
    ...
    for (j = 0; j < num_parents_y; j++) {
        for (i = 0; i < num_parents_x; i++) {
            ...
            World_Block *parent = &parents[parent_index];
            ...
            do_rewrite_rule(n0, parent);
            do_rewrite_rule(n1, parent);
            do_rewrite_rule(n2, parent);
            do_rewrite_rule(n3, parent);
        }
    }
...
*/
World_Block *make_static_terrain_block(Elevation_Map *map,
                                                int block_samples) {
    ...
    World_Block *nodes = new World_Block[num_blocks];
    int i, j;
    for (j = 0; j < num_blocks_y; j++) {
        for (i = 0; i < num_blocks_x; i++) {
            ...
            World_Block *block = &nodes[block_index];
            make_world_block(map, block, samples_x, samples_y, si_x, si_y);
    ...
    for (j = 0; j < num_blocks_y; j++) {
        for (i = 0; i < num_blocks_x; i++) {
            ...
            if (i < num_blocks_x - 1) {
                World_Block *block_plus_x = &nodes[block_index_plus_x];
            ...
            if (j < num_blocks_y - 1) {
                World_Block *block_plus_y = &nodes[block_index_plus_y];
    ...
    World_Block *root = build_tree_upward(nodes, num_blocks_x, num_blocks_y, block_id_offset + num_blocks_x * num_blocks_y);
