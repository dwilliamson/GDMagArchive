...
void Transformation_Matrix::zero_matrix() {
    int i, j;
...
void Transformation_Matrix::identity() {
    zero_matrix();
...
void Transformation_Matrix::scale_coefficients(double factor) {
    int i, j;
    for (j = 0; j < 3; j++) {
        for (i = 0; i < 3; i++) {
			coef[j][i] *= factor;
...
void Transformation_Matrix::set_rotation(const Quaternion &q) {
    Rotation_Matrix rm;
    rm.set(q);
...
void Transformation_Matrix::invert_33() {
    double adj[3][3];
    double a = coef[0][0];
    double b = coef[0][1];
    double c = coef[0][2];
...
void Transformation_Matrix::invert() {
...
void Transformation_Matrix::rotate(const Quaternion &q) {
    Transformation_Matrix rot_matrix;
    rot_matrix.identity();
    ...
    memcpy(this, &result, sizeof(Transformation_Matrix));
}

void Transformation_Matrix::translate(const Vector3 &t) {
    coef[0][3] += t.x;
    coef[1][3] += t.y;
    coef[2][3] += t.z;
}

void Transformation_Matrix::translate(double x, double y, double z) {
    coef[0][3] += x;
    coef[1][3] += y;
    coef[2][3] += z;
}

void Transformation_Matrix::scale(const Vector3 &v) {
    coef[0][0] *= v.x;
    coef[0][1] *= v.x;
    coef[0][2] *= v.x;
    coef[0][3] *= v.x;
...
void Transformation_Matrix::shear(const Vector3 &v) {
    Transformation_Matrix result;
    result.next = next;
...
void rotation_multiply(Rotation_Matrix *x, Rotation_Matrix *y,
                       Rotation_Matrix *r) {
    int i, j, k;
    double accum;
...
void Transformation_Matrix::get_translation(Vector3 *result) {
    result->x = (coef[0])[3];
    result->y = (coef[1])[3];
    result->z = (coef[2])[3];
}

void Transformation_Matrix::remove_translation(Vector3 *result) {
...
void Transformation_Matrix::copy_from(Transformation_Matrix *other) {
    memcpy(this, other, sizeof(Transformation_Matrix));
}
...
void Transformation_Matrix::get_orientation(Quaternion *result) {
...
// Cut-and-pasted from Transformation_Matrix::get_orientation
void Rotation_Matrix::get_orientation(Quaternion *result) {
    double trace = coef[0][0] + coef[1][1] + coef[2][2];
    double s;
    Quaternion q;
...
Vector3 Transformation_Matrix::transform(Vector3 const &p) {
    Vector3 result;
...
Vector3 Transformation_Matrix::transform_rotate_only(Vector3 const &p) {
    Vector3 result;
...
void Rotation_Matrix::concat(Rotation_Matrix *other) {
    int i, j, k;
    double accum;
...
void Transformation_Matrix::copy_33(Rotation_Matrix *rm) {
    int j, i;
    for (j = 0; j < 3; j++) {
        for (i = 0; i < 3; i++) {
...
void Rotation_Matrix::transpose() {
    double b = coef[0][1];
    double c = coef[0][2];
    double d = coef[1][0];
    double f = coef[1][2];
