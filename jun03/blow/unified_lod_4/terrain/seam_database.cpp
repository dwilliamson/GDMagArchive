...
/*
void Seam_Database::find_single_block_seams(Static_Terrain_Block *block, 
                                            List *results) {
    Mesh_Seam *seam;
    Array_Foreach(&seams, seam) {
        if (seam->block_membership[1] == NULL) results->add(seam);
    } Endeach;
}
*/

int block_id_difference(Mesh_Seam *seam1, Mesh_Seam *seam2, int index) {
    Static_Terrain_Block *block1 = seam1->block_membership[index];
    Static_Terrain_Block *block2 = seam2->block_membership[index];
...
void Seam_Database::collect_seams(Static_Terrain_Block *block, int n0, int n1,
                                  List *results) {
    int j;
    for (j = n0; j < n1; j++) {
...
Mesh_Seam *Seam_Database::rewrite_seam(Mesh_Seam *old_seam,
    ...
    int i;
    for (i = 0; i < 3; i++) {
        Static_Terrain_Block *other = old_seam->block_membership[i];
        if (!other) break;
...
void Seam_Database::perform_rewrite_rules(Auto_Array <Rewrite_Rule *> *rules,
                                          int max_hierarchy_distance,
                                          int starting_index) {
    int num_changes = 0;
    int old_num_seams = seams.live_items;

    Rewrite_Rule *rule;
    Array_Foreach(rules, rule) {
        List seams_to_rewrite;

        collect_seams(rule->source_block, starting_index, seams.live_items, &seams_to_rewrite);
...
void Seam_Database::find_seams_containing_these_blocks(Static_Terrain_Block **blocks, int num_blocks, 
