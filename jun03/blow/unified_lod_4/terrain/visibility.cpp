...
void add_appropriate_children(Auto_Array <Static_Terrain_Block *> *array,
                              Static_Terrain_Block *block) {
    int i;
    for (i = 0; i < MAX_CHILDREN; i++) {
        Static_Terrain_Block *child = block->children[i];
        if (!child) continue;
...
void collect_terrain(Static_Terrain_Block *block, 
    ...
    case I_AM_NOT_INVOLVED:
        for (i = 0; i < MAX_CHILDREN; i++) {
            collect_terrain(block->children[i], pieces,
...
// Entry point
void collect_visible_terrain(Static_Terrain_Block *block, 
                             Segregated_Terrain_Pieces *pieces) {
    if (block == NULL) return;
