...
Inefficient_Mesh::Inefficient_Mesh(int _num_vertices, int _num_faces) {
    num_vertices = _num_vertices;
    num_faces = _num_faces;

    vertices = new Vector3[num_vertices];
    face_vertices = new int[3 * num_faces];
    uvs = new Texture_Vertex[3 * num_faces];
    material_indices = new int[num_faces];

	material_info = NULL;
	num_materials = 0;
	tangent_frames = NULL;

    blend_info = new Vertex_Blend_Info[num_vertices];
    int i;
    for (i = 0; i < num_vertices; i++) {
        blend_info[i].num_matrices = 0;
    }
...
Inefficient_Mesh::~Inefficient_Mesh() {
	// This stuff is not being allocated all in one block because
	// this type of model is not supposed to be efficient;
	// it's more for ease of programming.  Before too long we
	// will have a triangle-stripped guy that will be all about
	// the optimization.
	
    delete [] face_vertices;
    delete [] material_indices;
    delete [] uvs;
    delete [] tangent_frames;
}
    
void Inefficient_Mesh::compute_normals() {
    // XXX Do this
}
