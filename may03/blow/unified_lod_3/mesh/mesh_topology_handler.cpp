...
/*
  Disabled for now because it messes up the seam stuff.
  If you want to perform this kind of operation it should
  be done as a pre-pass (the LODer probably won't create
  any new such faces except in pathological cases).  And
  if you really worry about that you can handle it in a
  post-pass that knows about seams.
