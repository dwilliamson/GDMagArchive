...
const float LENGTH_EPSILON = 0.0001;
...
void Mesh_Reducer::compensate_for_lonely_edge(Reducer_Face *face,
    ...
    float normal_len2 = normal.length();
    if (normal_len2 < LENGTH_EPSILON) return;  // Give up!
...
Mesh_Builder *Mesh_Reducer::prepare_result(int **remap) {
    ...
    // @Incomplete:  We should handle tangent frames here when we start using them through the pipeline     if (mesh->tangent_frames) smodel->tangent_frames = new Quaternion[num_faces * 3];
...
/*  @Incomplete: More potential tangent frame stuff
        smodel->tangent_frames[i*3+0] = output_tangent_frames[face->indices[0]];
        smodel->tangent_frames[i*3+1] = output_tangent_frames[face->indices[1]];
        smodel->tangent_frames[i*3+2] = output_tangent_frames[face->indices[2]];
*/
