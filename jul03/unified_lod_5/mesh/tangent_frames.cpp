...
Quaternion simple_rotation(const Vector3 &a, const Vector3 &b) {
    ...
    if (dot <= -1 + EPSILON) return Quaternion(1, 0, 0, 0);
...
/*
  ...
  However, I like this function a lot so I am leaving it
  in the source code!
...
Vector3 get_rotated_x_axis(const Quaternion &q) {
    return Vector3(1 - 2 * (q.y*q.y + q.z*q.z),
                   2 * (q.w*q.z + q.x*q.y),
                   2 * (q.x*q.z - q.w*q.y));
}
*/
...
void Tangent_Frame_Maker::complete_tangent_frames() {
    ...
    for (i = 0; i < num_vertices; i++) {
        ...
        Vector3 x_after_reach(1, 0, 0);  // x before reach
        x_after_reach.rotate(reach);     // x after reach

        // X was orthogonal to Z before the reach transform, so 
        // they must still be orthogonal afterward.  'tangent'
        // is also orthogonal to Z after the reach, because the
        // reach makes Z == 'normal', and of course 'normal' and
        // 'tangent' are orthogonal.  So the rotation that maps 
        // 'x_after_reach' to 'tangent' is in the plane orthogonal
        // to 'normal', that is, it's a twist rotation.
...
void make_matrix_from_points(Matrix3 *matrix,
                             Vector3 points[3]) {
    Vector3 v1, v2, v3;
...
Vector3 pull_a_tangent_out_of_our_ass(Vector3 normal) {
    ...
    Vector3 best_axis = x;
    float best_dot = fabs(dot_product(x, normal));

    float dot_y = fabs(dot_product(y, normal));
    float dot_z = fabs(dot_product(z, normal));
