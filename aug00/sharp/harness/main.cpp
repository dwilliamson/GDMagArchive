...
void init()
{
    ...
    GlobalCamera::Instance()->setFarPlane(220.0f);
...
int main(int argc, char** argv) {
    ...
    {
      if (strcmp(argv[x], "-window") == 0 || strcmp(argv[x], "/window") == 0)
      {
        fullscreen = false;
      }
    }
    ...
    sprintf(gameModeString, "%dx%d:%d", screenWidth, screenHeight, colorDepth);

    // Look for modestring overrides.
    for (x=1; x<argc-1; x++)
    {
      if (strcmp(argv[x], "-vidmode") == 0 || strcmp(argv[x], "/vidmode") == 0)
      {
        strcpy(gameModeString, argv[x+1]);
        sscanf(gameModeString, "%dx%d:%d", &screenWidth, &screenHeight, &colorDepth);
      }
    }
    ...
    std::cout << "==============================================" << std::endl;
    std::cout << "Demo Command-Line Switches:" << std::endl << "\t-window:\n\t\tRun in a window.";
    std::cout << std::endl << std::endl;
    std::cout << "\t-vidmode WIDTHxHEIGHT:COLORDEPTH : " << std::endl;
    std::cout << "\t\tSpecify the fullscreen video mode (only applicable" << std::endl;
    std::cout << "\t\tif not running windowed.)  Default is 800x600:16" << std::endl;
