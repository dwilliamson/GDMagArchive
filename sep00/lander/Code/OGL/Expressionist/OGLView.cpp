	...
	if (m_Model.vertex != NULL)
	{
		...
		for (loop = 0; loop < m_Model.faceCnt; loop++,face++)
		{
			for (loop2 = 0; loop2 < 3; loop2++)
			{
				...
				MAKEVECTOR(vect,m_Model.vertex[face->v[loop2]].x,m_Model.vertex[face->v[loop2]].y, m_Model.vertex[face->v[loop2]].z)
				NormalizeVector(&vect);
				m_Model.texture[face->t[loop2]].u = atan2(vect.x,vect.z) / PI_TIMES_TWO;
				m_Model.texture[face->t[loop2]].v = (m_Model.vertex[face->v[loop2]].y) * scale + 0.5 + offset;
			}
		}
	}
}
	...
	if (m_Model.vertex != NULL)
	{
		...
		for (loop = 0; loop < m_Model.faceCnt; loop++,face++)
		{
			for (loop2 = 0; loop2 < 3; loop2++)
			{
				...
				{
				case XY_PLANE:
					m_Model.texture[face->t[loop2]].u = (m_Model.vertex[face->v[loop2]].x) * scaleU + 0.5 + offsetU;
					m_Model.texture[face->t[loop2]].v = (m_Model.vertex[face->v[loop2]].y) * scaleV + 0.5 + offsetV;
					break;
				case XZ_PLANE:
					m_Model.texture[face->t[loop2]].u = (m_Model.vertex[face->v[loop2]].x) * scaleU + 0.5 + offsetU;
					m_Model.texture[face->t[loop2]].v = -(m_Model.vertex[face->v[loop2]].z) * scaleV + 0.5 + offsetV;
					break;
				case YZ_PLANE:
					m_Model.texture[face->t[loop2]].u = (m_Model.vertex[face->v[loop2]].z) * scaleU + 0.5 + offsetU;
					m_Model.texture[face->t[loop2]].v = (m_Model.vertex[face->v[loop2]].y) * scaleV + 0.5 + offsetV;
					break;
				}
			}
