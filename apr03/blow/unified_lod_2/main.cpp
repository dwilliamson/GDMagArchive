...
char *app_name = "Unified Rendering LOD #2";
...
/*
  ...
  that stuff.
*/
void draw_terrain_view_seam(Static_Terrain_Tree *tree_a, 
                            Static_Terrain_Tree *tree_b, 
...
// I am not going to explain this function in too much
// excruciating detail since it will change significantly
// before the final version of the code is done.  But it's
// not too hard to understand, you just need to dink around
// with a diagram of the 4-tree of nodes.
Static_Terrain_Tree *get_neighbor(Static_Terrain_Tree *node,
    ...
    // Are we looking for a neighbor block that is a lower
    // resolution than us?
    if (level_offset == -1) {
        if (direction == PLUS_X) {
    ...
    // Or are we looking for a neighbor block that is a higher
    // resolution than us?
    if (level_offset == 1) {
    ...
    // If neither of those, we must be looking for someone
...
// Another helper function, basically a wrapper around the
// get_neighbor function above.  Then once we have our neighbor
// block and have chosen the appropriate seam, we draw it.
void do_solid_seam_stuff(Static_Terrain_Tree *tree, Seam_Set *seam_set,
                         Neighbor_Direction direction, int level_offset) {
    ...
    if (neighbor->opacity == 1) {  // Only draw the seam between us if both I and my neighbor are opaque
        draw_terrain_view_seam(tree, neighbor, seam_a, level_offset + 1);
    }

    if (other_neighbor && (other_neighbor->opacity == 1)) {  // Only draw the seam between us if both I and my neighbor are opaque
...
void draw_solid_seams(Static_Terrain_Tree *tree) {
    // Ensure that we are up-to-date with the current transform...
    rendering_3d(client_globals.transformer->current_transform, client_globals.view_projector);

    // Look for all the various seam cases...
    do_solid_seam_stuff(tree, &tree->block->seam_sets[PLUS_X], PLUS_X, 0);
    do_solid_seam_stuff(tree, &tree->block->seam_sets[PLUS_Y], PLUS_Y, 0);
    do_solid_seam_stuff(tree, &tree->block->seam_sets[PLUS_X], PLUS_X, 1);
    do_solid_seam_stuff(tree, &tree->block->seam_sets[PLUS_Y], PLUS_Y, 1);
    do_solid_seam_stuff(tree, &tree->block->seam_sets[PLUS_X], PLUS_X, -1);
    do_solid_seam_stuff(tree, &tree->block->seam_sets[PLUS_Y], PLUS_Y, -1);
}
...
void switch_lod_color(Static_Terrain_Tree *tree, float alpha) {
    Clamp(alpha, 0, 1);
...
void set_child_properties(Static_Terrain_Tree *tree,
                          float opacity, int frame_index,
                          Lod_Instance_State state) {
    int i;
    for (i = 0; i < MAX_CHILDREN; i++) {
        Static_Terrain_Tree *child = tree->children[i];
        if (!child) continue;
...
int count_child_triangles(Static_Terrain_Tree *tree) {
    int sum = 0;
    ...
    for (i = 0; i < MAX_CHILDREN; i++) {
        ...
        sum += child->block->mesh->num_faces;
    }
...
/*
  ...
  here.  
*/
void select_terrain_lods(Static_Terrain_Tree *tree) {
    ...
    float r = tree->block->edge_length * 0.5f;
    ...
    if (t > 0) {
        if ((tree->leaf_distance >= 4) && (dist < 200)) {
            graham();
        }
    }
