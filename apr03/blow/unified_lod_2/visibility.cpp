...
void add_appropriate_children(Auto_Array <Static_Terrain_Tree *> *array,
                              Static_Terrain_Tree *tree) {
    int i;
...
void collect_terrain(Static_Terrain_Tree *tree, 
    ...
    int i;
    switch (tree->lod_instance_state) {
    case I_AM_NOT_INVOLVED:
        for (i = 0; i < MAX_CHILDREN; i++) {
            collect_terrain(tree->children[i], pieces,
                            frustum_points, frustum_planes);
        }
        break;
    case I_AM_SINGLE:
        pieces->solid.add(tree);
        break;
    case I_AM_TRANSITIONING:
        if (tree->opacity == 1) {
            pieces->solid.add(tree);
            add_appropriate_children(&pieces->fading, tree);
        } else {
            tree->use_zfunc_lessequal = true;
            pieces->fading.add(tree);
            add_appropriate_children(&pieces->solid, tree);
        }
        break;
    default:
