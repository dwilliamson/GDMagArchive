...
void fill_result(Triangle_List_Mesh *result, Triangle_List_Mesh *source,
                 int vertex_cursor, int index_cursor, int triangle_list_cursor, Vector3 offset, Vector3 uv_offset, int *index_map) {
    int i;
...
/*
void xref_indices(Triangle_List_Mesh *orig_mesh, int *index_map,
                  Mesh_Reducer *reducer) {

    int i;
    for (i = 0; i < orig_mesh->num_vertices; i++) {
        int old_index = index_map[i];
        index_map[i] = reducer->topology_handler->old_index_to_new_index[old_index];
    }
}
*/
...
void set_child(Static_Terrain_Tree *parent, int index, Static_Terrain_Tree *child) {
    ...
    if (child) {
        ...
        assert((void *)child->block != (void *)0xfdfdfdfd);
    }
}
...
Static_Terrain_Tree *make_static_terrain_tree(Elevation_Map *map,
    ...
    for (j = 0; j < num_blocks_y; j++) {
        for (i = 0; i < num_blocks_x; i++) {
            ...
            block->mesh->material_info[0].name = app_strdup("data/bitmaps/3grass001"); // @Memory: Maybe don't do this strdup all the time.
        }
    }
