...
void collect_terrain(Static_Terrain_Tree *tree, 
                     Auto_Array <Static_Terrain_Tree *> *array,
                     Vector3 *frustum_points, Plane3 *frustum_planes) {

    if (tree == NULL) return;

    bool visible = box_versus_frustum(tree->block->bounding_box_corner + tree->block->position, tree->block->bounding_box_extents, frustum_points, frustum_planes);
    if (!visible) return;

    if (tree->lod_instance_state == RENDER_MY_CHILDREN) {
        collect_terrain(tree->children[0], array, frustum_points, frustum_planes);
        collect_terrain(tree->children[1], array, frustum_points, frustum_planes); 
        collect_terrain(tree->children[2], array, frustum_points, frustum_planes);
        collect_terrain(tree->children[3], array, frustum_points, frustum_planes);
...
// Entry point
void collect_terrain(Static_Terrain_Tree *tree, Auto_Array <Static_Terrain_Tree *> *array) {
    if (tree == NULL) return;
    ...
    collect_terrain(tree, array, frustum_points, frustum_planes);
}
