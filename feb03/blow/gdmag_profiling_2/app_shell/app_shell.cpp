...
const int USING_D3D_AND_NOT_OPENGL = 0;
...
double busy_double = 1;


void App_Shell::sleep2(float seconds) {
/*
    int i;
    for (i = 0; i < ITERATIONS * seconds; i++) {
...
int PASCAL WinMain(HINSTANCE hinst, HINSTANCE,
    ...
    // We need to init the profiler before doing anything else,
    // because App_Shell::init opens a window which will
    // call the WindowProc which can invoke the profiler.
    // Man, Windows is annoying.
    extern void gamelib_profile_init();
    gamelib_profile_init();
