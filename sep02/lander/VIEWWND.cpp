...
/////////////////////////////////////////////////////////////////////////////////////
	...
	switch (message)
		...
		case WM_LBUTTONDOWN:
			...
			// simple snap to grid if desired
			if (g_GridSnap)
			{
				g_MouseHitX = ((g_MouseHitX + 20) / 40) * 40;
				g_MouseHitY = ((g_MouseHitY + 20) / 40) * 40;
			}
		...
		case WM_MOUSEMOVE:
			...
			if (g_ForceDrag)
			{
				...
				localY = CVector(g_ViewMatrix[1],g_ViewMatrix[5],g_ViewMatrix[9]);

				localX = CVector(g_ViewMatrix[0],g_ViewMatrix[4],g_ViewMatrix[8]);

				SetMouseForce(g_MouseHitX -tx,g_MouseHitY - ty,&localX,&localY);
				g_MouseForceActive = TRUE;
			}
			else if (g_Dragging)
				ChangeGround(tx,g_ScreenHeight - ty - 1);
		...
		case WM_LBUTTONUP:
			...
			if (g_Pick[0] == -1 && g_Pick[1] == -1)
				AddParticle(tx,g_ScreenHeight - ty - 1);
			g_ForceDrag = FALSE;			
