...
void Covariance_Matrix2::reset() {
	a = b = c = 0;
}

Covariance_Matrix2 Covariance_Matrix2::invert() {
    double det = a*c - b*b;
    double factor = 1.0 / det;

    Covariance_Matrix2 result;
    result.a = c * factor;
    result.b = -b * factor;
    result.c = a * factor;
...
Covariance_Matrix2 Covariance_Matrix2::add(const Covariance_Matrix2 &other) {
    Covariance_Matrix2 result;
    result.a = a + other.a;
    result.b = b + other.b;
    result.c = c + other.c;
...
void Covariance_Matrix2::scale(float factor) {
	a *= factor;
	b *= factor;
	c *= factor;
}

void Covariance_Matrix2::rotate(float theta) {
	double s = sin(theta);
	double t = cos(theta);
...
// The Covariance_Matrix2 eigenvector path is completely separate (I wrote
// it first, and it was simpler to just crank out).  

int Covariance_Matrix2::find_eigenvalues(float eigenvalues[2]) {
    double qa, qb, qc;
    qa = 1;
    qb = -(a + c);
    qc = a * c - b * b;
...
int Covariance_Matrix2::find_eigenvectors(float eigenvalues[2], Vector3 eigenvectors[2]) {
...
void Covariance_Matrix2::move_to_global_coordinates(Covariance_Matrix2 *dest, float x, float y) {
    dest->a = a + x*x;
    dest->b = b + x*y;
    dest->c = c + y*y;
}

void Covariance_Matrix2::move_to_local_coordinates(Covariance_Matrix2 *dest, float x, float y) {
    dest->a = a - x*x;
    dest->b = b - x*y;
    dest->c = c - y*y;
}
