...
Ast_Declarative_Assertion *find_assertion(Ast *expression) {
    ...
    if (expression->type == AST_ARRAY_SUBSCRIPT) return ((Ast_Array_Subscript *)expression)->right;
    return NULL;
}
...
int Lerp_Bytecode_Builder::emit_code_for_array_subscript(Ast_Array_Subscript *sub) {
    //
    ...
    // short-circuit this.
    int right_register = emit_code_for_optimized_sequence_formation(sub->right);
    int result_register = allocate_output_register();
...
int Lerp_Bytecode_Builder::emit_code_for_assignment(Ast_Expression *lvalue_expression, int rvalue_register) {
    ...
    } else if (lvalue_expression->type == AST_ARRAY_SUBSCRIPT) {
        Ast_Array_Subscript *sub = (Ast_Array_Subscript *)lvalue_expression;
        int left_register = emit_code_for_expression(sub->left);
        int right_register = emit_code_for_optimized_sequence_formation(sub->right);
...
*/
    ...
    case AST_ARRAY_SUBSCRIPT:
        result = emit_code_for_array_subscript((Ast_Array_Subscript *)expression);
        break;
    case AST_BLOCK:
