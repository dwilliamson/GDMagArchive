...
First_Class *Lerp_Memory_Manager::gc_move(First_Class *oldspace_fc) {
    ...
    case ARG_IMPERATIVE_PROCEDURE: {
        ...
        COPY(proc);
        COPY(t_self_start);
        COPY(total_self_ticks);
        COPY(total_entry_count);
        COPY(average_seconds);
        POINTER(bytecode);
        result = newspace;
        break;
    }
...
void Lerp_Memory_Manager::gc() {
    ...
    // Enumerate root set
    //
    extern void glib_enumerate_gc_roots(Lerp_Interp *interp);
    glib_enumerate_gc_roots(interp);


    interp->global_database = (Database *)gc_consider(interp->global_database);
    interp->bytecode_runner->current_context = ToBarrier((Lerp_Call_Record *)gc_consider(interp->bytecode_runner->current_context));

    Integer_Hash_Table *table = interp->custom_types_by_id;
    int i;
    for (i = 0; i < table->table_size; i++) {
        List *l = &table->lists[i];
        Integer_Hash_Info *info;
        Foreach(l, info) {
            Barrier <Schema *> *value = (Barrier <Schema *> *)info->value;
            info->value = gc_consider(value);
        } Endeach;
    }
