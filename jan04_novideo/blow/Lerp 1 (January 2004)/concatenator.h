...
void add(char);  // Needed for packing binary code stuff!
    void add(char *);
    void add(int);
    ...
    // a 0 to the data.
    char *get_nozeroterm_result(unsigned int *len_return);

    // 'reset' erases the current result; calling this->length() will
    // then return 0.
    void reset();

    Concatenator(int block_size = DEFAULT_CONCAT_BLOCK_SIZE);
    ~Concatenator();
