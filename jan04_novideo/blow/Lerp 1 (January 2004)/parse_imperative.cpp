...
Ast_Expression *Parser::parse_imperative_expression_helper() {
    ...
    if (token->type == '[') {
        ...
        Ast_Array_Subscript *result = AST_NEW(Ast_Array_Subscript);
        result->left = left;
    ...
    if (is_binary_operator(token)) {
        ...
        if ((right->type == AST_PROCEDURE_CALL_EXPRESSION) && (binary_token_type == '.')) {
        ...
        result->type = AST_BINARY_EXPRESSION;
        ...
        if (right->type == AST_BINARY_EXPRESSION) {
            Ast_Binary_Expression *bin_r = (Ast_Binary_Expression *)right;
