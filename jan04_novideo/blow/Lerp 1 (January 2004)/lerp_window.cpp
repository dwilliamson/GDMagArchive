...
Window_Lib *glib;  // A global... eventually this will be in a DLL so it's okay.
// XXXXXXXXX Need to eliminate the global list of windows (iterate through db dude!)
...
Private_Window_Blob *get_blob(Lerp_Interp *interp, Database *owner) {
    ...
    if (blob_fc->type != ARG_BLOB) return NULL;
...
extern void add_db_assertion(Database *db, Decl_Assertion *assertion);
Database *make_singleton_database(Lerp_Interp *interp, Decl_Expression *expression) {
    Database *db = GC_NEW(Database);
    Decl_Assertion *assertion = make_singleton_assertion(interp, expression);
...
void bind_value(Lerp_Interp *interp, char *name_string, First_Class *value) {
    ...
    interp->global_database->assign_named_slot(interp, name, value);
}
...
*/
...

    {
        ...
        bind_value(interp, "Mouse_Pointer", glib->Mouse_Pointer);
    }
...

    {
        ...
        // one for each font character, giving the character info...
        // there is no way to represent that yet in the Schema.  @Incomplete
        bind_value(interp, "Font_Character_Info", ns);
    ...
    bind_value(interp, "Keyboard", glib->Keyboard);
...
#define TRACE(p) *(First_Class **)&(glib->p) = mm->gc_consider(glib->p);

void glib_enumerate_gc_roots(Lerp_Interp *interp) {
    Lerp_Memory_Manager *mm = interp->memory_manager;

    TRACE(Window_namespace);
    TRACE(Mouse_Pointer);
    TRACE(Keyboard);
    TRACE(Windows);
    TRACE(window_string);
    TRACE(key_held_down_string);
    TRACE(key_pressed_string);
    TRACE(key_released_string);
    TRACE(text_input_string);
    TRACE(char_string);

    TRACE(buffer_for_keyboard);

    int i;
    for (i = 0; i < NUM_KEY_CODES; i++) {
        String *s = glib->key_names[i];
        if (s) glib->key_names[i] = (String *)mm->gc_consider(s);
    }

    First_Class *win;
    Foreach(&glib->list_of_windows, win) {
        First_Class *result = mm->gc_consider(win);
