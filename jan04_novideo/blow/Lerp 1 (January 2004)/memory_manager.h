...
struct Lerp_Memory_Manager {
    ...
    void *allocate(int num_bytes, int type_id = 0);

    void print_allocations();


    Lerp_Call_Record *create_call_record(int num_registers);
    void release_call_record(Lerp_Call_Record *record);

    Decl_Expression *create_decl_expression(int num_arguments);
    Lerp_Bytecode *create_bytecode(int num_constants);

    Value_Array *create_value_array(int num_values);

    void gc();

    Memory_Arena *memory_oldspace, *memory_newspace;
    First_Class *gc_consider(void *ptr);
