...
// Created:
//		JL 11/20/98		
//		JL 06/12/00		Modified from other code
///////////////////////////////////////////////////////////////////////////////
//
...
//    glEnable(GL_DEPTH_TEST);	// ENABLE DEPTH TESTING
	...
	if (m_SimRunning)
		m_PhysEnv.m_FrameCnt ++;

	if (m_CurObject)
		m_PhysEnv.RenderWorld((tVector *)m_CurObject->visuals->vertexData);		// DRAW THE SIMULATION
	else
		m_PhysEnv.RenderWorld(NULL);		// Draw the world box at least

	if (m_DrawGeometry && m_CurObject)
		DrawModel(m_CurObject);
...
void COGLView::HandleKeyUp(UINT nChar) 
{
	...
	{
	case 'S':
		m_PhysEnv.AddSpring(FALSE);
		break;
	case 'M':
		m_PhysEnv.AddSpring(TRUE);
		break;
	case 'C':
		m_PhysEnv.m_MusclesFlexin = !m_PhysEnv.m_MusclesFlexin;
		break;
	case 'E':
		m_PhysEnv.EditMuscle();
		break;
	case 'G':
...
void COGLView::OnMouseMove(UINT nFlags, CPoint point) 
{
	...
	if (nFlags & MK_LBUTTON > 0)
	{
		...
		if ((nFlags & MK_CONTROL) > 0 && m_CurObject != NULL)
		{
		}	
...
///////////////////////////////////////////////////////////////////////////////
	...
	if (ext == "OBJ")
	{
				...
				LOADOBJ_VERTEXONLY | LOADOBJ_REUSEVERTICES))
		{
			...
			m_CurObject = &children[m_Skeleton.childCnt];
			ResetBone(m_CurObject,&m_Skeleton);
			strcpy(m_CurObject->name,(LPCTSTR)baseName);
			m_CurObject->visuals = visual;
			m_CurObject->visualCnt = 1;
			...
			if (fp != NULL)
			{
				...
				if (m_Skeleton.childCnt > 0)
				{
					...
					if (m_Skeleton.children->visualCnt > 0)
					{
						...
						m_CurObject = m_Skeleton.children;
					}
				}
				fclose(fp);
			}
