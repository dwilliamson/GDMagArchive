...
extern LRESULT MaxHandleWindowMessage( HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam );
...
// 
//-----------------------------------------------------------------------------
void pauseRenderer(bool b)
{
   if (b)
   {
      g_d3dApp.SetActive(false);
      ShowWindow(g_d3dApp.GetMainWindow(), FALSE);
      SuspendThread(ghThread);
   }
   else
   {
      ResumeThread(ghThread);
      ShowWindow(g_d3dApp.GetMainWindow(), TRUE);
      g_d3dApp.SetActive(TRUE);
      InvalidateRect(g_d3dApp.GetMainWindow(), 0, TRUE);
   }
}
...
// 
//-----------------------------------------------------------------------------
HWND createRenderWindow(HINSTANCE hInst, HWND hParent, int x, int y, int w, int h)
{
   // create window
   ...
   if (!h)
    ...
    if( FAILED( g_d3dApp.Create( hInst, hParent, x, y, w, h ) ) )
        return 0;

    // create and run thread

    ghThread = CreateThread(0, 0, ThreadProc, 0, 0, &gdwThreadID);
...
//-----------------------------------------------------------------------------
// CMyD3DApplication::loadAndDisplayMesh
//-----------------------------------------------------------------------------
HRESULT CMyD3DApplication::loadAndDisplayMesh(const char *pFilename)
{
   ...
   strcpy(m_szPath, pFilename);

   return LoadMeshHierarchy();
...
//   if (bOnce)
//   {
//      lstrcpy(m_szPath, "C:\\DXSDK\\samples\\Multimedia\\Media\\car.x");
...
/*
   HRSRC hsrc = FindResource(0, MAKEINTRESOURCE(IDD_SHADER1), RT_RCDATA);
   ...
   char msg[255];
	...
	FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM, NULL,
						  ...
						  msg, sizeof(msg), NULL );// Display the string.

   HGLOBAL hg = LoadResource(0, hsrc);
...
*/
    ...
    for (DWORD i = 0; i < 4; ++i)
    {
        ...
        if( FAILED( hr = D3DXAssembleShaderFromResource(ghInstance, MAKEINTRESOURCE(IDD_SHADER1 + i), 0, NULL, &pCode, NULL ) ) )
        {
           char msg[255];
           D3DXGetErrorString(hr, msg, sizeof(msg));
            return hr;
        }
...
//-----------------------------------------------------------------------------
// CMyD3DApplication::HandleMessage
//-----------------------------------------------------------------------------
LRESULT CMyD3DApplication::HandleMessage( HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam )
{
   return MaxHandleWindowMessage(hWnd, uMsg, wParam, lParam );
